...
function bindMenuActions(){ const install=()=>{if(window.deferredPrompt){window.deferredPrompt.prompt();window.deferredPrompt=null;return;}if(/iPhone|iPad|iPod/i.test(navigator.userAgent)){alert("Pà installà l'appiicazioni :\n\n1. Pichjati nantu Sparta (Partager)\n2. Pichjati nantu à ⌄ Veda di più (en voir plus)\n3. Scigliti Nantu screnu d'accolta (Sur l'écran d'accueil)");return;}alert("L'installazioni ùn hè micca dispuniibili cù stu navigatori.");}; $('btnInstall')?.addEventListener('click',()=>{ closeMenu(); install(); }); $('btnHistory')?.addEventListener('click',()=>{ closeMenu(); showHistory(); }); $('btnChange')?.addEventListener('click',()=>{ closeMenu(); localStorage.removeItem('username'); localStorage.removeItem('entity'); location.href='index.html'; }); }
