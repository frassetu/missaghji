...
function bindMenuActions(){ const install=()=>{if(window.deferredPrompt){window.deferredPrompt.prompt();window.deferredPrompt=null;return;}if(/iPhone|iPad|iPod/i.test(navigator.userAgent)){alert("Pour installer l'application :\n\n1. Appuyez sur Partager\n2. Choisissez 'Sur l'écran d'accueil'");return;}alert("L'installation n'est pas disponible sur ce navigateur.");}; $('btnInstall')?.addEventListener('click',()=>{ closeMenu(); install(); }); $('btnHistory')?.addEventListener('click',()=>{ closeMenu(); showHistory(); }); $('btnChange')?.addEventListener('click',()=>{ closeMenu(); localStorage.removeItem('username'); localStorage.removeItem('entity'); location.href='index.html'; }); }
