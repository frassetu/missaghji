...
(function(){
  ...
  function configureByRole(){
    ...
    hide(emFuncSel); hide(reFuncSel); show(emFunc); show(reFunc);
    ...
    if(role==='emetteur'){
      ...
      // Use SELECT instead of free input for collaborator's function
      setOptions(reFuncSel, COLLAB_FUNCS); hide(reFunc); show(reFuncSel);
      ...
      setOptions(emFuncSel, COLLAB_FUNCS); hide(emFunc); show(emFuncSel);
  ...
  function gotoAuth(ev){ if(ev){ ev.preventDefault(); ev.stopPropagation(); } closeMenu(); location.hash='#/auth'; // force
    // Show immediately and focus to avoid waiting for hashchange
    document.body.classList.add('only-auth'); views.auth.classList.remove('hidden'); views.app.classList.add('hidden'); views.history.classList.add('hidden');
    setTimeout(()=>{ $('authEntity').focus(); },0);
  }
  ...
  $('btnSave').addEventListener('click', ()=>{ const user=ls.get(STORAGE.user,null); if(!user){ alert("Identifiez-vous d'abord"); location.hash='#/auth'; route(); return; } const time=`${(hh.value||'').padStart(2,'0')}:${(mm.value||'').padStart(2,'0')}`; const rec={ time, emNum:(emNum.value||'').trim(), emName:(emName.value||'').trim().toUpperCase(), emFunc:(emFunc.value||'').trim(), emEntity: emEntity.disabled ? (ls.get(STORAGE.user,null)||{}).entity : emEntity.value, reNum:(reNum.value||'').trim(), reName:(reName.value||'').trim().toUpperCase(), reFunc: ( (roleSelect.value==='emetteur')? (reFuncSel.value||'') : (reFunc.value||'') ).trim(), reEntity: reEntity.disabled ? (ls.get(STORAGE.user,null)||{}).entity : reEntity.value, message:(message.value||'').trim() };
    ...
    // ✅ Rafraîchir la page (formulaire) après enregistrement
    doRefresh();
  });
