...
(function(){
  ...
  function counterpartEntities(userEntity){ if(userEntity==='BAO') return GROUP_ENTITIES.concat(['DELCO']); const group=new Set(GROUP_ENTITIES); if(group.has(userEntity)) return ['BAO','DELCO']; return ['BAO','DELCO']; }
  ...
  function configureByRole(){
    ...
    if(role==='emetteur'){
      ...
      // collaborator (recepteur) - empty fields and dynamic entity options
      reName.value=''; reFunc.value='';
      setCollaboratorEntityOptions();
      // generators
  ...
  function setCollaboratorEntityOptions(){
    const user=ls.get(STORAGE.user,null); if(!user) return; const role=roleSelect.value;
    const funcField = (role==='emetteur')? reFunc : emFunc;
    const entitySelect = (role==='emetteur')? reEntity : emEntity;

    const f = funcField.value.trim().toUpperCase();
    if(user.entity==='BAO'){
      // If I'm BAO: collaborator function is one of CCO, CDC, CDT, CDR, PDM, PDA, PDE, PDS (datalist already). We don't prefill it.
      if(f==='CCO'){
      ...
      // Not BAO: legacy rule: collaborator entity BAO by default, DELCO if function is CCO
  ...
  // update collaborator entity options when function changes
  reFunc.addEventListener('input', setCollaboratorEntityOptions);
  emFunc.addEventListener('input', setCollaboratorEntityOptions);
  ...
  // Change entity (go to auth)
  function gotoAuth(e){ if(e){ e.preventDefault(); e.stopPropagation(); } closeMenu(); location.hash = '#/auth'; show('auth'); setTimeout(()=>{ window.scrollTo(0,0); $('authEntity').focus(); }, 0); }
