...
function showHistory(){ const list=JSON.parse(localStorage.getItem('history')||'[]'); const c=$('content'); if(list.length===0){ c.innerHTML=`<section class="card"><h2>Storicu</h2><p>Nisunu missaghju arragistratu.</p><div class="actions"><button class="btn" id="back">Ritornu</button></div></section>`; $('back').onclick=()=>renderApp(); return; } const out=list.map(x=>`${x.date} ${x.time}\nEmettori: N° ${x.em_num} | ${x.em_name} | ${x.em_func} | ${x.em_ent}\nRicivitori: N° ${x.re_num} | ${x.re_name} | ${x.re_func} | ${x.re_ent}\n${x.msg}\n-----`).join("\n"); c.innerHTML=`<section class="card"><h2>Storicu</h2><pre>${out}</pre><div class="actions"><button class="btn primary" id="exportCsv">Spurtà in CSV</button><button class="btn ghost" id="clearHistory">Squassà tuttu</button><button class="btn" id="back">Ritornu</button></div></section>`; $('back').onclick=()=>renderApp(); $('exportCsv').onclick=exportCSV; $('clearHistory').onclick=()=>{ if(confirm("Seti sicuru di sguassà tuttu ?")){ localStorage.removeItem('history'); renderApp(); } }; bindMenuActions(); }
